...
- on or multiple changes: run effect
...
export default function App() {
  const currentPage = useSyncExternalStore(
    ...
    () => window?.location.hash // reand the value (server)
  );
...
export default function useHashChange() {
  return useSyncExternalStore(
    ...
    () => window.location.hash, // How to ge value on client
    () => window?.location.hash // How to get the value on the server
  );
}
```
